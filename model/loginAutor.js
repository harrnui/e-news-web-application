...
async function login(data, client, response){
    ...
    if (getPWandRole.rows.length === 0){
        ...
        if(getPWandRole.rows[0].role_id === 2){
            try{
                const provjeraLozinke = await bcrypt.compare(data.lozinka, getPWandRole.rows[0].password);
                
    
                if(provjeraLozinke){
                    response.redirect("/autorOpcije");//need to send in browser a notification that the user logged in
                }else{
            ...
            response.send("You don't have autor privileges.");
        }
